...
import { generationExitCode, loopRunExitCode, type OutputMode, writeError, writeResult } from './output.js';
import {
  buildGenerationCreateRequest,
...
export const runCli = async (argv: string[], context: CLIContext = {}): Promise<number> => {
...
export const createProgram = (runtime: Runtime): Command => {
...
const registerProjects = (program: Command, runtime: Runtime): void => {
  ...
  withMutation(projects.command('create <website-url>'))
    ...
    .action(async (websiteUrl: string, _options: unknown, command: Command) => {
      ...
      if ('status' in result && ['failed', 'archived'].includes(result.status)) runtime.exitCode = 3;
    });
  ...
  projects.command('get <project-id>').action(async (projectId: string) => {
    const client = await clientFor(program, runtime);
  ...
  withMutation(projects.command('update <project-id>'))
    ...
    .action(async (projectId: string, _options: unknown, command: Command) => {
      ...
      if (opts.patch !== undefined && explicitFields) throw new CLIUsageError('--patch cannot be mixed with update flags.');
      const patch = opts.patch !== undefined
        ? await readJsonReference<Record<string, unknown>>(opts.patch)
        : compact({
      ...
      const client = await clientFor(program, runtime);
      writeResult(
        await client.projects.update(projectId, patch as never, mutationOptions(program, command)),
        outputMode(program, runtime)
      );
    });

  withMutation(projects.command('refresh <project-id>')).action(async (projectId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.projects.refresh(projectId, mutationOptions(program, command)), outputMode(program, runtime));
  });

  projects.command('archive <project-id>').action(async (projectId: string) => {
    const client = await clientFor(program, runtime);
  ...
  withMutation(logo.command('set <project-id> [file]'))
    ...
    .action(async (projectId: string, file: string | undefined, _options: unknown, command: Command) => {
      const opts = command.opts();
      ...
      const client = await clientFor(program, runtime);
      writeResult(await client.projects.setLogo(projectId, input as never, mutationOptions(program, command)), outputMode(program, runtime));
    });
  logo.command('rm <project-id>').action(async (projectId: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.projects.deleteLogo(projectId, requestOptions(program)), outputMode(program, runtime));
  });
};

const registerKnowledge = (program: Command, runtime: Runtime): void => {
  ...
  withPage(collections.command('list')).requiredOption('--project <project-id>').action(async (_options: unknown, command: Command) => {
    const opts = command.opts();
    const client = await clientFor(program, runtime);
  ...
  collections.command('rename <collection-id> <name>').action(async (collectionId: string, name: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.collections.update(collectionId, { name }, requestOptions(program)), outputMode(program, runtime));
  });
  collections.command('rm <collection-id>').action(async (collectionId: string) => {
    const client = await clientFor(program, runtime);
  ...
  withMutation(sources.command('add [input]'))
    ...
    .action(async (input: string | undefined, _options: unknown, command: Command) => {
      ...
      const client = await clientFor(program, runtime);
      const result = await client.sources.create(opts.project, sourceInput as never, mutationOptions(program, command));
      writeResult(result, outputMode(program, runtime), result.id);
    });
  ...
  sources.command('get <source-id>').action(async (sourceId: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.sources.get(sourceId, requestOptions(program)), outputMode(program, runtime));
  });
  withMutation(sources.command('refresh <source-id>')).action(async (sourceId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.sources.refresh(sourceId, mutationOptions(program, command)), outputMode(program, runtime));
  });
  sources.command('rm <source-id>').action(async (sourceId: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.sources.delete(sourceId, requestOptions(program)), outputMode(program, runtime));
  });
};

const registerDiscovery = (program: Command, runtime: Runtime): void => {
  const assetTypes = program.command('asset-types');
  withPage(assetTypes.command('list')).action(async (_options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.assetTypes.list(pageOptions(command), requestOptions(program)), outputMode(program, runtime));
  });
  assetTypes.command('show <asset-type>').action(async (assetType: string) => {
    const client = await clientFor(program, runtime);
    const result = await findAcrossPages<AssetTypeDefinition>(
      cursor => client.assetTypes.list({ ...(cursor === undefined ? {} : { cursor }), limit: 100 }, requestOptions(program)),
      item => item.id === assetType
    );
    if (result === null) throw new CLIUsageError(`Asset type ${assetType} was not found in the live catalog.`);
    writeResult(result, outputMode(program, runtime));
  });

  const models = program.command('models');
  withPage(models.command('list')).requiredOption('--asset-type <asset-type>').action(async (_options: unknown, command: Command) => {
    const opts = command.opts();
    const client = await clientFor(program, runtime);
  ...
  models.command('show <model-id>').requiredOption('--asset-type <asset-type>').action(async (modelId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    const assetType = command.opts().assetType;
    const result = await findAcrossPages<Model>(
      cursor => client.models.list({ asset_type: assetType, ...(cursor === undefined ? {} : { cursor }), limit: 100 }, requestOptions(program)),
      item => item.id === modelId
    );
    if (result === null) throw new CLIUsageError(`Model ${modelId} was not found for ${assetType}.`);
    writeResult(result, outputMode(program, runtime));
  });
...
const registerMediaResource = (program: Command, runtime: Runtime, family: 'voices' | 'avatars'): void => {
  ...
  withPage(group.command('list'))
    ...
    .action(async (_options: unknown, command: Command) => {
      ...
      writeResult(await resource.list(compact({
        ...
        status: opts.status,
        ...pageOptions(command)
      }) as never, requestOptions(program)), outputMode(program, runtime));
    });
  withMutation(group.command('create [file]'))
    ...
    .action(async (file: string | undefined, _options: unknown, command: Command) => {
      ...
      const client = await clientFor(program, runtime);
      const result = await client[family].create(opts.project, input as never, mutationOptions(program, command));
      writeResult(result, outputMode(program, runtime), result.id);
    });
  group.command('get <id>').action(async (id: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client[family].get(id, requestOptions(program)), outputMode(program, runtime));
  });
  group.command('rm <id>').action(async (id: string) => {
    const client = await clientFor(program, runtime);
...
const registerGenerationCommands = (program: Command, runtime: Runtime): void => {
  ...
  generate.action(async (_options: unknown, command: Command) => {
    const opts = command.opts() as GenerationFlags & Record<string, unknown>;
    validateWaitAndOutput(opts, command);
    const draft = await buildGenerationDraft(opts);
    const client = await clientFor(program, runtime);
    let fallbackMaxCost: string | undefined;
    if (isInteractive(runtime)) {
      ...
      if (opts.yes !== true && !await confirm(`Generate for up to $${opts.maxCost ?? fallbackMaxCost}?`, runtime)) return;
  ...
  generations.command('get <generation-id>').action(async (generationId: string) => {
    const client = await clientFor(program, runtime);
  ...
  generations.command('wait <generation-id>')
    ...
    .option('--output <path>')
    .option('--output-dir <directory>')
    .action(async (generationId: string, _options: unknown, command: Command) => {
      const opts = command.opts();
      if (opts.output !== undefined && opts.outputDir !== undefined) throw new CLIUsageError('Use either --output or --output-dir.');
      const client = await clientFor(program, runtime);
  ...
  generations.command('cancel <generation-id>').action(async (generationId: string) => {
    const client = await clientFor(program, runtime);
  ...
  generations.command('edit-preview <generation-id>')
    ...
    .action(async (generationId: string, _options: unknown, command: Command) => {
      const input = await readJsonReference<GenerationEditDraft>(command.opts().request);
      const client = await clientFor(program, runtime);
  ...
  withMutation(generations.command('edit <generation-id>'))
    ...
    .action(async (generationId: string, _options: unknown, command: Command) => {
      const opts = command.opts();
      const input = await readJsonReference<Record<string, unknown>>(opts.request);
      ...
      if (cap === undefined) throw new CLIUsageError('Generation edit requires --max-cost or max_cost_usd.');
      const result = await client.generations.edit(
        generationId,
        { ...input, max_cost_usd: cap } as GenerationEditRequest,
        mutationOptions(program, command)
      );
      writeResult(result, outputMode(program, runtime), result.id);
    });
};

const registerAssets = (program: Command, runtime: Runtime): void => {
  ...
  assets.command('get <asset-id>').action(async (assetId: string) => {
    const client = await clientFor(program, runtime);
  ...
  assets.command('feedback <asset-id>')
    ...
    .action(async (assetId: string, _options: unknown, command: Command) => {
      const opts = command.opts();
...
const registerLoops = (program: Command, runtime: Runtime): void => {
  ...
  withMutation(addGenerationFlags(loops.command('create')))
    ...
    .action(async (_options: unknown, command: Command) => {
      const opts = command.opts();
      let input: ContentLoopCreateInput;
      if (opts.request !== undefined) {
        ...
        if (mixed) throw new CLIUsageError('--request cannot be mixed with Loop request-building flags.');
        input = await readJsonReference<ContentLoopCreateInput>(opts.request);
      } else {
        ...
        const draft = await buildGenerationDraft(opts as GenerationFlags);
        input = {
  ...
  loops.command('get <loop-id>').action(async (loopId: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.contentLoops.get(loopId, requestOptions(program)), outputMode(program, runtime));
  });
  withMutation(loops.command('update <loop-id>')).requiredOption('--request <@file>').action(async (loopId: string, _options: unknown, command: Command) => {
    const input = await readJsonReference<ContentLoopUpdateInput>(command.opts().request);
    const client = await clientFor(program, runtime);
    writeResult(await client.contentLoops.update(loopId, input, mutationOptions(program, command)), outputMode(program, runtime));
  });
  withMutation(loops.command('pause <loop-id>')).action(async (loopId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.contentLoops.pause(loopId, mutationOptions(program, command)), outputMode(program, runtime));
  });
  withMutation(loops.command('resume <loop-id>')).action(async (loopId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.contentLoops.resume(loopId, mutationOptions(program, command)), outputMode(program, runtime));
  });
  withMutation(loops.command('run <loop-id>')).requiredOption('--max-cost <usd>').action(async (loopId: string, _options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
  ...
  withPage(runs).action(async (loopId: string | undefined, _options: unknown, command: Command) => {
    if (loopId === undefined) throw new CLIUsageError('loops runs requires a Loop ID, or use loops runs get <run-id>.');
    const client = await clientFor(program, runtime);
    writeResult(await client.contentLoops.runs(loopId, pageOptions(command), requestOptions(program)), outputMode(program, runtime));
  });
  runs.command('get <run-id>').action(async (runId: string) => {
    const client = await clientFor(program, runtime);
...
const registerWebhooks = (program: Command, runtime: Runtime): void => {
  const hooks = program.command('webhooks');
  withMutation(hooks.command('add <url>')).action(async (url: string, _options: unknown, command: Command) => {
    const mode = outputMode(program, runtime);
    if (mode.quiet) throw new CLIUsageError('--quiet cannot be used when creating a webhook because its secret is returned once.');
    const client = await clientFor(program, runtime);
    const result = await client.webhooks.create({ url }, mutationOptions(program, command));
    writeResult(result, mode);
    if ('secret_unavailable' in result && result.secret_unavailable === true) runtime.exitCode = 3;
  });
  withPage(hooks.command('list')).action(async (_options: unknown, command: Command) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.webhooks.list(pageOptions(command), requestOptions(program)), outputMode(program, runtime));
  });
  hooks.command('rm <webhook-id>').action(async (webhookId: string) => {
    const client = await clientFor(program, runtime);
    writeResult(await client.webhooks.delete(webhookId, requestOptions(program)), outputMode(program, runtime));
  });
};

const addGenerationFlags = (command: Command): Command => command
  .option('--request <@file>')
  .option('--project <project-id>')
  .option('--trend')
  .option('--topic <text>')
...
const withMutation = (command: Command): Command => command
  .option('--idempotency-key <key>');

const clientFor = async (program: Command, runtime: Runtime): Promise<AutoContent> => {
  const apiKey = await loadApiKey(runtime.environment);
  if (apiKey === null) throw new CLIUsageError(`No Platform API key found. Create one at ${API_KEYS_URL}, then run autocontent login.`);
...
const outputMode = (program: Command, runtime: Runtime): OutputMode => {
  const opts = program.opts();
  return {
...
const sourceCreateInput = async (
  ...
  if (input === undefined) throw new CLIUsageError('A Source input is required.');
...
const readStream = async (stream: NodeJS.ReadableStream): Promise<string> => {
  const chunks: Buffer[] = [];
  for await (const chunk of stream as NodeJS.ReadableStream & AsyncIterable<unknown>) {
...
const findAcrossPages = async <T>(
  load: (cursor?: string) => Promise<{ data: T[]; next_cursor: string | null }>,
...
const confirm = async (question: string, runtime: Runtime): Promise<boolean> => {
...
const isInteractive = (runtime: Runtime): boolean => streamIsTTY(runtime.stdin) === true && streamIsTTY(runtime.stdout) === true;

const streamIsTTY = (stream: NodeJS.ReadableStream | NodeJS.WritableStream): boolean | undefined =>
  (stream as NodeJS.ReadableStream & { isTTY?: boolean }).isTTY;

const validateWaitAndOutput = (opts: Record<string, unknown>, command: Command): void => {
...
const hasEmbeddedCap = (draft: GenerationDraft): boolean =>
  typeof (draft as GenerationDraft & { max_cost_usd?: unknown }).max_cost_usd === 'string';

const boundedInteger = (value: unknown, minimum: number, maximum: number, name: string): number => {
...
const nullableId = (value: unknown): string | null | undefined =>
  value === undefined ? undefined : value === 'none' ? null : String(value);
...
const omit = (value: Record<string, unknown>, key: string): Record<string, unknown> =>
  Object.fromEntries(Object.entries(value).filter(([current]) => current !== key));

...
test('concrete repeated outputs use deterministic plan endpoints without starting inference', async () => {
  ...
  const plan = { project_id: 'prj_test', items: [
    { id: 'out_first', brief: 'First deliberate hook', input: { type: 'topic' as const, topic: 'A useful lesson' }, asset: { asset_type: 'short_video' as const, options: { duration_seconds: 30, captions: false } } },
    { id: 'out_second', brief: 'Second deliberate hook', input: { type: 'topic' as const, topic: 'A useful lesson' }, asset: { asset_type: 'short_video' as const, language: 'fr-FR', options: { duration_seconds: 30 } } }
  ] };
...
test('managed messages preserve captured page and caller request identity', async () => {
  ...
  await client.agent.send({ message: 'Explain this page', inference_max_cost_usd: '0.10',
    page: { tab_id: 'd3e3df9f-8111-4e9a-8c0a-20599180bff8', view_id: '1b116b82-8bbe-4cb2-9650-bf657c7403fd',
      ...
      filters: {}, actions: [], draft: null, proposal: null }, attachment_source_ids: [] },

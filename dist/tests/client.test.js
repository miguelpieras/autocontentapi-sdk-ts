...
import AutoContent, { AmbiguousUploadError, InvalidRequestError, MaxCostExceededError, constructWebhookEvent, extensionAsset } from '../index.js';
test('client requires exactly one authentication mode', () => {
    assert.throws(() => new AutoContent({}), /exactly one/u);
...
test('one-shot upload transport loss is surfaced as ambiguous and is not replayed', async () => {
    ...
    await assert.rejects(client.sources.create('prj_one', {
        file: Readable.from(['one shot']),
        filename: 'notes.txt'
    }), (error) => error instanceof AmbiguousUploadError
...
test('API errors are normalized into typed classes', async () => {
    ...
    await assert.rejects(client.generations.create({
        project_id: 'prj_one',
        input: { type: 'knowledge' },
        assets: [{ asset_type: 'article' }],
        max_cost_usd: '2.00'
    }), (error) => error instanceof MaxCostExceededError
...
test('OAuth-only methods fail locally when configured with an API key', () => {
    ...
    assert.throws(() => client.apiKeys.create({ name: 'CI', scopes: ['platform.read'] }), InvalidRequestError);
});

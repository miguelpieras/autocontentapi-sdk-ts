...
export const apiKeyConfigPath = (environment = process.env) => {
    if (platform() === 'win32') {
...
export const storeApiKey = async (apiKey, environment = process.env) => {
    const normalized = apiKey.trim();
    if (!normalized.startsWith('acp_'))
        throw new TypeError('Expected a Platform API key beginning with acp_.');
    ...
    const body = `${JSON.stringify({ api_key: normalized })}\n`;
...
export const loadApiKey = async (environment = process.env) => {
